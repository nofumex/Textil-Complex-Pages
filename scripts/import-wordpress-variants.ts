...
async function importWordPressVariants() {
  console.log('📥 Импорт вариаций товаров из WordPress XML...\n');
  ...
  try {
    ...
    if (sampleVariants.length > 0) {
      ...
      sampleVariants.forEach(variant => {
        console.log(`  - ${variant.product.title} | ${variant.color || 'Без цвета'} | ${variant.size || 'Без размера'} | ${variant.price} руб.`);
      });
    }
